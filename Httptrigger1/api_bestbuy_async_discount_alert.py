...
lumberjack = logging.getLogger(__name__ + " - httptrigger1 - bestbuy deals")
...
def error_msg(e):
    ...
    subject = f'HttpTrigger1 - Best Buy Products Error ({datetime.now()})'
    body = f'''\
error: {e}
...
def status_msg(df_total, df_disc, last_update_date):
    ...
    # * email subject and body
    subject = f'HttpTrigger1 - Best Buy Deals ({datetime.now()})'
...
async def bb_main(last_update_date=_config_bestbuy.last_update_date, page_size=100, batch_size=4, test=False, email=False):
...
if __name__ == '__main__':
    ...
    try:
        loop.run_until_complete(bb_main(page_size=100, batch_size=4, test=True, email=False))
