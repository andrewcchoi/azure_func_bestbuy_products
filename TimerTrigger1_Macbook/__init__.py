...
def async_call(url: str, subject: str, columns: List=None, detail_names: List=None, email: bool=False) -> None:
    ...
    .. code-block:: python
        ...
        email = True
        df, df_shape = async_call(url, subject, columns, detail_names, email)
        print(df)
        print(df_shape)
    """
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    loop.run_until_complete(bb_main(url=url, subject=subject, columns=columns, detail_names=detail_names, email=email))
...
def main(mytimer: func.TimerRequest) -> None:
    ...
    for query in queries.keys():
        async_call(
            ...
            , details_name=queries[query]["detail_names"]
            , email=True
        )
